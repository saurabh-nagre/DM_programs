...
# st.write('The current number is ', number)
    ...
    if operation == 'DIANA':
        ...
        plt.figure(figsize=(20, 15))
        dendrogram(dist_sin, above_threshold_color='#070dde',
                   orientation='bottom', leaf_rotation=90)
        plt.xlabel('Index')
        plt.ylabel('Distance')
    ...
    if operation == "DBSCAN":
        ...
        for i in range(len(arr1)):
            ...
            tmp.append(arr2[i])
            dataset.append(tmp)
        r = st.number_input('Insert value for eps', value=0.1)
        mnp = st.number_input(
            'Insert mimimum number of points in cluster', step=1, value=5)
        ...
        button = st.button("Find Clusters")
        if button:
            calculate()
